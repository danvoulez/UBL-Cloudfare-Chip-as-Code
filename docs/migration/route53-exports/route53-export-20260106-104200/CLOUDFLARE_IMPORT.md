...
```bash
# Via API
curl -X POST "https://api.cloudflare.com/client/v4/zones"   -H "Authorization: Bearer $CF_API_TOKEN"   -H "Content-Type: application/json"   --data '{"name":"logline.foundation","account":{"id":"$CLOUDFLARE_ACCOUNT_ID"}}'
```
...
aws route53 get-hosted-zone --id Z1006717337OSEG7ZZM7N
...
# Verificar registros
dig logline.foundation A +short
```
...
# aws route53 delete-hosted-zone --id Z1006717337OSEG7ZZM7N
```
