use axum::{routing::{get, post, any}, Router, extract::{State, Path}, http::{HeaderMap, StatusCode}, body::Bytes};
    ...
    async fn main() -> anyhow::Result<()> {
        let upstream = std::env::var("UPSTREAM").unwrap_or_else(|_| "http://127.0.0.1:9453".into());
        let pubkey_pem_b64 = std::env::var("POLICY_PUBKEY_PEM_B64").expect("set POLICY_PUBKEY_PEM_B64");
        let policy_yaml_path = std::env::var("POLICY_YAML").unwrap_or("/etc/ubl/nova/policy/ubl_core_v1.yaml".into());
        let pack_json_path = std::env::var("POLICY_PACK").unwrap_or("/etc/ubl/nova/policy/pack.json".into());
    ...
    fn load_and_verify(policy_yaml_path: &str, pack_json_path: &str, pubkey_pem_b64: &str) -> anyhow::Result<SemanticChip> {
        ...
        let pem_bytes = general_purpose::STANDARD.decode(pubkey_pem_b64)?;
        let vk = VerifyingKey::from_public_key_pem(std::str::from_utf8(&pem_bytes).unwrap())?;
        let sig_b64 = pack.get("signature").and_then(|v| v.as_str()).ok_or(anyhow::anyhow!("signature missing"))?;
        let sig = Signature::from_bytes(&general_purpose::STANDARD.decode(sig_b64)?);
    ...
    async fn reload(State(state): State<AppState>) -> Result<String, (StatusCode, String)> {
        let chip = load_and_verify(&state.policy_yaml_path, &state.pack_json_path, &state.pubkey_pem_b64)
            .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
        *state.chip.write() = chip;
        Ok("{\"ok\":true,\"reloaded\":true}".into())
    }
    ...
    async fn forward(
        ...
        let ctx = RequestContext {
            ...
            did: Some(format!("{} /{}", req.method(), path)),
        ...
        fwd = fwd.headers(pass).body(body);
    ...
    fn append_ledger(line: &str) -> std::io::Result<()> {
        ...
        let p = "/var/log/ubl/nova-ledger.ndjson";
