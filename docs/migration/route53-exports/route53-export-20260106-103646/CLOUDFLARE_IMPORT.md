...
```bash
# Via API
curl -X POST "https://api.cloudflare.com/client/v4/zones"   -H "Authorization: Bearer $CF_API_TOKEN"   -H "Content-Type: application/json"   --data '{"name":"voulezvous.tv","account":{"id":"$CLOUDFLARE_ACCOUNT_ID"}}'
```
...
aws route53 get-hosted-zone --id Z05567431BSXG2C2U6RNS
...
dig voulezvous.tv NS +short

# Verificar registros
dig voulezvous.tv A +short
```
...
# aws route53 delete-hosted-zone --id Z05567431BSXG2C2U6RNS
```
